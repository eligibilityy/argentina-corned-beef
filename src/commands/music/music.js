...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const initChannel = guild.channels.cache.filter(chnl => (chnl.type == ChannelType.GuildVoice)).find(channel => (channel.members.has(client.user.id)))
        if(test && voiceChannel.id !== test.id) return interaction.reply({embeds: [Response.setDescription(`I'm already playing in <#${initChannel.id}>.`)], ephemeral: true})
